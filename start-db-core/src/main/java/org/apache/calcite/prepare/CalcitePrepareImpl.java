...
public class CalcitePrepareImpl implements CalcitePrepare {
    ...
    public static final boolean ENABLE_ENUMERABLE = CalciteSystemProperty.ENABLE_ENUMERABLE.value();

    @Deprecated // to be removed before 2.0
    public static final boolean ENABLE_STREAM = CalciteSystemProperty.ENABLE_STREAM.value();

    @Deprecated // to be removed before 2.0
    public static final List<RelOptRule> ENUMERABLE_RULES = EnumerableRules.ENUMERABLE_RULES;
    ...
    private static final Set<String> SIMPLE_SQLS = ImmutableSet.of(
        "SELECT 1",
        "select 1",
        "SELECT 1 FROM DUAL",
        "select 1 from dual",
    ...
    private ParseResult convert_(
        ...
        final JavaTypeFactory typeFactory = context.getTypeFactory();
        final Convention resultConvention = enableBindable
        ...
        final CalcitePreparingStmt preparingStmt = new CalcitePreparingStmt(
            this,
            context,
            catalogReader,
            typeFactory,
        ...
        final SqlToRelConverter converter = preparingStmt.getSqlToRelConverter(
            validator,
            catalogReader,
            config
        );
    ...
    private AnalyzeViewResult analyze_(
        SqlValidator validator,
        String sql,
        SqlNode sqlNode,
        RelRoot root,
        ...
        for (RelDataTypeField field : targetRowType.getFieldList()) {
            ...
            if (fail) {
                throw validator.newValidationError(
                    sqlNode,
                    RESOURCE.noValueSuppliedForViewColumn(field.getName(), Util.last(tablePath))
                );
            }
            return new AnalyzeViewResult(
                this,
    ...
    public void executeDdl(Context context, SqlNode node) {
        ...
        final SqlParserImplFactory parserFactory = config.parserFactory(
    ...
    public <T> CalciteSignature<T> prepareQueryable(Context context, Queryable<T> queryable) {
    ...
    private <T> CalciteSignature<T> simplePrepare(Context context, String sql) {
        final JavaTypeFactory typeFactory = context.getTypeFactory();
        final RelDataType x = typeFactory.builder()
    ...
    private SqlValidator createSqlValidator(Context context, CalciteCatalogReader catalogReader) {
        final SqlOperatorTable opTab0 = context.config()
        ...
        final SqlValidator.Config config = SqlValidator.Config.DEFAULT.withLenientOperatorLookup(
            connectionConfig.lenientOperatorLookup()
        )
    ...
    private ColumnMetaData.AvaticaType avaticaType(
        JavaTypeFactory typeFactory,
        RelDataType type,
        RelDataType fieldType
    ) {
        ...
        if (type.getComponentType() != null) {
            final ColumnMetaData.AvaticaType componentType = avaticaType(
                typeFactory,
    ...
    protected void populateMaterializations(
        Context context,
        ...
        try {
            ...
            final CalciteMaterializer materializer = new CalciteMaterializer(
                this,
                context,
                catalogReader,
                schema,
            ...
            throw new RuntimeException(
                "While populating materialization " + materialization.materializedTable.path(),
                e
            );
        }
    }
    ...
    public <R> R perform(
        ...
        final CalciteSchema schema = config.getDefaultSchema() != null
     ...
     */
    static class CalcitePreparingStmt extends Prepare implements RelOptTable.ViewExpander {
        ...
        public PreparedResult prepareQueryable(final Queryable queryable, RelDataType resultType) {
            return prepare_(() -> {
                ...
                return new LixToRelTranslator(cluster, CalcitePreparingStmt.this).translate(
                    queryable
                );
            }, resultType);
        }
        ...
        private PreparedResult prepare_(Supplier<RelNode> fn, RelDataType resultType) {
            ...
            final List<Pair<Integer, String>> fields = Pair.zip(
        ...
        protected PreparedResult implement(RelRoot root) {
            ...
            if (resultConvention == BindableConvention.INSTANCE) {
                ...
                try {
                    ...
                    bindable = EnumerableInterpretable.toBindable(
                        internalParameters,
                        context.spark(),
                        enumerable,
                        prefer
        ...
        protected List<Materialization> getMaterializations() {
            final List<Materialization> materializations = context.config()
        ...
        public RexNode toRex(Expression expression) {
            switch (expression.getNodeType()) {
                ...
                case Call:
                    ...
                    SqlOperator operator = RexToLixTranslator.JAVA_TO_SQL_METHOD_MAP.get(
                        call.method
                    );
                    if (operator != null) {
